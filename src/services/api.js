...
const LAST_ACTIVITY_STORAGE_KEY = "deymake.auth.lastActivityAt";
...
export function getStoredActivityAt() {
...
export function setStoredActivityAt(value) {
  if (!value) {
    localStorage.removeItem(LAST_ACTIVITY_STORAGE_KEY);
    return null;
  }
  ...
  const normalizedValue = new Date(timestamp).toISOString();
  localStorage.setItem(LAST_ACTIVITY_STORAGE_KEY, normalizedValue);
  return normalizedValue;
}

export function touchStoredActivity(value = new Date()) {
  return setStoredActivityAt(value instanceof Date ? value.toISOString() : value);
}
...
async function request(path, options = {}) {
  ...
  const activityAt = token ? getStoredActivityAt() : null;
...
export const api = {
  ...
  sendPhoneCode: (payload) => request("/auth/send-phone-code", { method: "POST", body: payload }),
  verifyPhoneCode: (payload) => request("/auth/verify-phone-code", { method: "POST", body: payload }),
  registerWithPhone: (payload) => request("/auth/register-with-phone", { method: "POST", body: payload }),
  sendPhoneLoginCode: (payload) => request("/auth/send-phone-login-code", { method: "POST", body: payload }),
  loginWithPhone: (payload) => request("/auth/login-with-phone", { method: "POST", body: payload }),
  ...
  getAdminVideoReports: (options = {}) => request(`/admin/reports/videos${buildQueryString({ status: options.status, page: options.page, per_page: options.perPage })}`),
  updateAdminVideoReport: (id, payload) => request(`/admin/reports/videos/${id}`, { method: "PATCH", body: payload }),
  getAdminCreatorVerificationRequests: (options = {}) => request(`/admin/creator-verification-requests${buildQueryString({ status: options.status, page: options.page, per_page: options.perPage })}`),
  updateAdminCreatorVerificationRequest: (id, payload) => request(`/admin/creator-verification-requests/${id}`, { method: "PATCH", body: payload }),
  getAdminModerationCases: (options = {}) => request(`/admin/moderation/cases${buildQueryString({ status: options.status, contentType: options.contentType, riskLevel: options.riskLevel, page: options.page, per_page: options.perPage })}`),
  getAdminModerationCase: (id) => request(`/admin/moderation/cases/${id}`),
  updateAdminModerationCase: (id, payload) => request(`/admin/moderation/cases/${id}`, { method: "PATCH", body: payload }),
  getChallenges: (options = {}) => request(`/challenges${buildQueryString({ q: options.q, status: options.status, page: options.page, per_page: options.perPage })}`),
  getAdminVideos: (options = {}) => request(`/admin/videos${buildQueryString({ q: options.q, status: options.status, type: options.type, sort: options.sort, page: options.page, per_page: options.perPage })}`),
  getAdminVideo: (id) => request(`/admin/videos/${id}`),
  updateAdminVideo: (id, payload) => request(`/admin/videos/${id}`, { method: "PATCH", body: payload }),
  deleteAdminVideo: (id) => request(`/admin/videos/${id}`, { method: "DELETE" }),
  getAdminChallenges: (options = {}) => request(`/admin/challenges${buildQueryString({ q: options.q, status: options.status, sort: options.sort, page: options.page, per_page: options.perPage })}`),
  getAdminChallenge: (id) => request(`/admin/challenges/${id}`),
  updateAdminChallenge: (id, payload) => request(`/admin/challenges/${id}`, { method: "PATCH", body: payload }),
  deleteAdminChallenge: (id) => request(`/admin/challenges/${id}`, { method: "DELETE" }),
  ...
  repostVideo: (id) => request(`/videos/${id}/repost`, { method: "POST" }),
  unrepostVideo: (id) => request(`/videos/${id}/repost`, { method: "DELETE" }),
  ...
  getPayoutAccount: () => request("/monetization/payout-account"),
  savePayoutAccount: (payload) => request("/monetization/payout-account", { method: "PUT", body: payload }),
  getMonetizationPayouts: (options = {}) => request(`/monetization/payouts${buildQueryString({ status: options.status })}`),
  requestPayout: (payload) => request("/monetization/payouts", { method: "POST", body: payload }),
  getMonetizationTransactions: (options = {}) => request(`/monetization/transactions${buildQueryString({ status: options.status, type: options.type, page: options.page, per_page: options.perPage })}`),
  getTipsReceived: () => request("/tips/received"),
  getTipsSent: () => request("/tips/sent"),
  sendCreatorTip: (creatorId, payload) => request(`/creators/${creatorId}/tips`, { method: "POST", body: payload }),
  sendLiveTip: (videoId, payload) => request(`/videos/${videoId}/live/tips`, { method: "POST", body: payload }),
  getRevenueShares: (options = {}) => request(`/revenue-shares${buildQueryString({ scope: options.scope, page: options.page, per_page: options.perPage })}`),
  createRevenueShare: (payload) => request("/revenue-shares", { method: "POST", body: payload }),
  updateRevenueShare: (id, payload) => request(`/revenue-shares/${id}`, { method: "PATCH", body: payload }),
  createRevenueShareSettlement: (id, payload) => request(`/revenue-shares/${id}/settlements`, { method: "POST", body: payload }),
  getBrandCampaigns: () => request("/brand/campaigns"),
  createBrandCampaign: (payload) => request("/brand/campaigns", { method: "POST", body: payload }),
  updateBrandCampaign: (id, payload) => request(`/brand/campaigns/${id}`, { method: "PATCH", body: payload }),
  getBrandCampaignMatches: (id, options = {}) => request(`/brand/campaigns/${id}/matches${buildQueryString({ q: options.q, categoryId: options.categoryId, verifiedOnly: options.verifiedOnly ? 1 : undefined, minSubscribers: options.minSubscribers, hasActivePlans: options.hasActivePlans ? 1 : undefined, page: options.page, per_page: options.perPage })}`),
  getSponsorshipProposals: (options = {}) => request(`/sponsorships/proposals${buildQueryString({ scope: options.scope })}`),
  createSponsorshipProposal: (payload) => request("/sponsorships/proposals", { method: "POST", body: payload }),
  updateSponsorshipProposal: (id, payload) => request(`/sponsorships/proposals/${id}`, { method: "PATCH", body: payload }),
  getUserMerch: (id) => request(`/users/${id}/merch`),
  createMerchProduct: (payload) => request("/merch/products", { method: "POST", body: payload }),
  updateMerchProduct: (id, payload) => request(`/merch/products/${id}`, { method: "PATCH", body: payload }),
  createMerchOrder: (id, payload) => request(`/merch/products/${id}/orders`, { method: "POST", body: payload }),
  getMyMerchOrders: () => request("/merch/orders/mine"),
  getReceivedMerchOrders: () => request("/merch/orders/received"),
  updateMerchOrder: (id, payload) => request(`/merch/orders/${id}`, { method: "PATCH", body: payload }),
  getAcademyCourses: () => request("/academy/courses"),
  getAcademyCourse: (id) => request(`/academy/courses/${id}`),
  enrollInAcademyCourse: (id) => request(`/academy/courses/${id}/enroll`, { method: "POST" }),
  completeAcademyLesson: (id) => request(`/academy/lessons/${id}/complete`, { method: "POST" }),
  getMyAcademyLearning: () => request("/academy/me"),
  getAiStudioProjects: () => request("/ai/studio/projects"),
  getAiStudioProject: (id) => request(`/ai/studio/projects/${id}`),
  createAiStudioProject: (payload) => request("/ai/studio/projects", { method: "POST", body: payload }),
  generateAiStudioProject: (id) => request(`/ai/studio/projects/${id}/generate`, { method: "POST" }),
  getOfflineUploadQueue: () => request("/uploads/offline-queue"),
  saveOfflineUploadItem: (payload) => request("/uploads/offline-queue", { method: "POST", body: payload }),
  updateOfflineUploadItem: (id, payload) => request(`/uploads/offline-queue/${id}`, { method: "PATCH", body: payload }),
  getTalentDiscovery: (options = {}) => request(`/talent/discovery${buildQueryString({ q: options.q, categoryId: options.categoryId, verifiedOnly: options.verifiedOnly ? 1 : undefined, minSubscribers: options.minSubscribers, hasActivePlans: options.hasActivePlans ? 1 : undefined, page: options.page, per_page: options.perPage })}`),

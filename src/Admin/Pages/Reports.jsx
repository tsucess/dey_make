    ...
    <div className="flex flex-col gap-6 text-white font-inter">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex flex-col">
          ...
          <p className="text-sm text-slate-400 mt-1">Review and take action on content and user reports.</p>
        </div>
      ...
      <div className="grid grid-col-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 overflow-x-auto pb-2 gap-4 hide-scrollbar">
        {stats.map((stat, i) => (
          <div key={i} className="bg-[#0b1011] border border-zinc-800 rounded-xl p-5 flex flex-col gap-3 min-w-[200px] flex-1">
            <h3 className="text-xs text-slate-400 font-semibold">{stat.title}</h3>
            <span className="text-3xl font-semibold">{stat.value}</span>
            <div className={`flex items-center gap-1 text-[11px] font-medium ${stat.trendUp ? 'text-green-500' : 'text-red-500'}`}>
      ...
      {/* Tabs */}
      <div className="flex items-center gap-6 border-b border-zinc-800 pb-[1px] overflow-x-auto hide-scrollbar">
        {tabs.map((tab) => (
          <button
            key={tab.id}
          ...
          >
            {tab.name}
            {activeTab === tab.id && (
              <div className="absolute bottom-0 left-0 w-full h-[2px] bg-red-600 rounded-t-sm" />
            )}
          </button>
        ))}
      </div>
      ...
      <div className="flex flex-wrap items-end gap-3">
        <div className="flex items-center gap-2 px-4 py-2.5 bg-transparent border border-zinc-800 rounded-lg flex-1 min-w-[250px] h-[42px]">
          <FiSearch className="text-zinc-500 w-4 h-4 shrink-0" />
          <input
            type="text"
            value={searchQuery}
        ...
        {Object.keys(filterOptions).map((filter) => (
          <div key={filter} className="relative flex flex-col gap-1.5">
            <span className="text-xs text-slate-400 ml-1">{filter}</span>
            <button 
              onClick={() => setOpenDropdown(openDropdown === filter ? null : filter)}
              className="flex items-center justify-between gap-3 px-4 py-2 bg-transparent border border-zinc-800 rounded-lg text-sm text-slate-300 min-w-[160px] h-[42px]"
            >
              <span className="truncate">{filters[filter]}</span>
              <IoIosArrowDown className={`text-zinc-400 w-4 h-4 shrink-0 transition-transform ${openDropdown === filter ? 'rotate-180' : ''}`} />
            </button>
            {openDropdown === filter && (
              <div className="absolute top-full left-0 mt-2 w-full bg-[#131313] border border-zinc-800 rounded-lg shadow-xl z-20 py-1 max-h-60 overflow-y-auto">
        ...
        <div className="flex flex-col gap-1.5">
          <span className="text-xs text-slate-400 ml-1">Date</span>
          <button className="flex items-center justify-between gap-3 px-4 py-2 bg-transparent border border-zinc-800 rounded-lg text-sm text-slate-400 min-w-[180px] h-[42px]">
      ...
      {/* Table Area */}
      <div className="bg-[#0b1011] border border-zinc-800 rounded-xl overflow-x-auto mt-2 pb-2">
        {/* Desktop Table */}
        <table className="hidden md:table w-full text-left border-collapse min-w-[1000px]">
          <thead>
            <tr className="border-b border-zinc-800 bg-transparent">
              ...
              <th className="p-4 text-xs font-medium text-slate-300">Report ID</th>
              <th className="p-4 text-xs font-medium text-slate-300">Content</th>
              <th className="p-4 text-xs font-medium text-slate-300">Type</th>
              <th className="p-4 text-xs font-medium text-slate-300">Reason</th>
              <th className="p-4 text-xs font-medium text-slate-300">Reporter</th>
              <th className="p-4 text-xs font-medium text-slate-300">Status</th>
              <th className="p-4 text-xs font-medium text-slate-300">Reported At</th>
              <th className="p-4 text-xs font-medium text-slate-300 text-center">Action</th>
            </tr>
          </thead>
          <tbody>
            {filteredReports.map((report, i) => (
              <tr 
                ...
                className={`border-b border-zinc-800 hover:bg-zinc-800/30 transition-colors cursor-pointer ${selectedReport === report ? 'bg-zinc-800/20 border-l-2 border-l-[#4facfe]' : 'border-l-2 border-l-transparent'}`}
              >
                <td className="p-4 pl-6" onClick={(e) => e.stopPropagation()}>
                  <input type="checkbox" className="w-4 h-4 rounded border-zinc-600 bg-transparent accent-orange100 cursor-pointer" />
                </td>
                <td className="p-4 text-sm text-slate-300">{report.id}</td>
                <td className="p-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col">
                      ...
                      <span className="text-[10px] text-slate-500">{report.contentId}</span>
                    </div>
                  </div>
                </td>
                <td className={`p-4 text-sm font-medium ${report.type === 'Video' ? 'text-[#CB3CFF]' : 'text-red-500'}`}>
                  {report.type}
                </td>
                <td className="p-4 text-sm text-slate-300">{report.reason}</td>
                <td className="p-4">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex flex-col">
                      ...
                      <span className="text-[10px] text-slate-500">{report.reporterHandle}</span>
                    </div>
                  </div>
                </td>
                <td className="p-4">
                ...
                <td className="p-4 text-sm text-slate-300">{report.reportedAt}</td>
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-y-3 gap-x-2 pt-3 border-t border-zinc-800">
                <div className="flex flex-col gap-1">
                  <span className="text-[10px] text-slate-500 font-semibold uppercase tracking-wider">Type</span>
                  <span className={`text-xs font-medium ${report.type === 'Video' ? 'text-[#CB3CFF]' : 'text-red-500'}`}>{report.type}</span>
                </div>
                <div className="flex flex-col gap-1">
                  <span className="text-[10px] text-slate-500 font-semibold uppercase tracking-wider">Reason</span>
                  <span className="text-xs text-slate-300">{report.reason}</span>
                </div>
                <div className="flex flex-col gap-1">
                  <span className="text-[10px] text-slate-500 font-semibold uppercase tracking-wider">Reporter</span>
                  <div className="flex items-center gap-1.5 mt-0.5">
                    <img src="/story3.jpg" alt="" className="w-4 h-4 rounded-full object-cover" />
                    <span className="text-xs text-slate-300">{report.reporterName}</span>
                  </div>
                </div>
                <div className="flex flex-col gap-1">
        ...
        {/* Pagination */}
        <div className="p-4 px-6 flex items-center justify-between border-t border-zinc-800 mt-4">

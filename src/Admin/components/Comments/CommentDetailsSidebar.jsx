  ...
  return (
    <>
      ...
      <div className="fixed inset-y-0 right-0 z-50 w-full max-w-[450px] bg-blue400 border-l border-zinc-800 shadow-2xl flex flex-col transform transition-transform duration-300 ease-in-out translate-x-0">
        ...
        {/* Header */}
        <div className="flex justify-between items-center p-6 pb-4 shrink-0 border-b border-zinc-800/50">
        ...
        <div className="flex-1 overflow-y-auto p-6 pt-6 font-inter" style={{ scrollbarWidth: "none", msOverflowStyle: "none" }}>
          ...
          <div className="flex flex-col items-center mb-8">
            ...
            <p className="text-xs text-slate-400 mt-1">{comment.user.userType || "Creator"}</p>
            <p className="text-xs text-slate-400 mt-1">{comment.user.followers || "24M followers"}</p>
            
            <div className="mt-4 bg-[#3b1212] text-red-500 px-4 py-1.5 rounded-md text-xs font-medium border border-red-500/20">
              Hate Speech
            </div>
          ...
          <div className="bg-blue300 rounded-xl p-5 mb-5 border border-zinc-800/50">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex flex-col gap-1">
                ...
                <span className="text-xs text-slate-400">{comment.video?.id || "UXX-2224-839423"}</span>
          ...
          <div className="bg-blue300 rounded-xl p-5 mb-5 border border-zinc-800/50">
            ...
            <div className="flex flex-col">
              <div className="flex justify-between items-center py-3.5 border-b border-zinc-800">
                <span className="text-slate-400 text-xs font-medium">Comment ID</span>
                <span className="text-white text-xs font-medium">{comment.id}</span>
              </div>
              <div className="flex justify-between items-center py-3.5 border-b border-zinc-800">
                <span className="text-slate-400 text-xs font-medium">Posted On</span>
              ...
              <div className="flex justify-between items-center py-3.5 border-b border-zinc-800">
                ...
                <span className={`text-xs font-medium ${
                  ...
                  comment.status === 'Pending Review' ? 'text-orange-500' :
                  'text-red-500'
                }`}>
                  {comment.status}
                </span>
              ...
              <div className="flex justify-between items-center py-3.5 border-b border-zinc-800">
                <span className="text-slate-400 text-xs font-medium">Reported By</span>
                <span className="text-white text-xs font-medium">{comment.reportedBy}</span>
              </div>
              <div className="flex justify-between items-center py-3.5 border-b border-zinc-800">
                <span className="text-slate-400 text-xs font-medium">Device</span>
                <span className="text-white text-xs font-medium">{comment.device || "Android, Lagos, Nigeria"}</span>
              </div>
              <div className="flex justify-between items-center py-3.5 border-b border-zinc-800">
                <span className="text-slate-400 text-xs font-medium">IP Address</span>
              ...
              <div className="flex justify-between items-center py-3.5">
                <span className="text-slate-400 text-xs font-medium">Replies</span>
          ...
          {/* Reported Reasons */}
          <div className="bg-blue300 rounded-xl p-5 mb-6 border border-zinc-800/50">
            ...
            <div className="flex flex-col gap-4">
              {comment.reportedReasons?.map((reason, idx) => (
                <div key={idx} className="flex justify-between items-center">
                  <span className={`text-xs font-medium ${
                    reason.reason === 'Hate Speech' ? 'text-red-500' :
              ...
              {!comment.reportedReasons && (
                <>
                  <div className="flex justify-between items-center">
                    <span className="text-red-500 text-xs font-medium">Hate Speech</span>
          ...
          <div className="flex flex-col gap-3 pb-6">
            <button className="w-full flex items-center justify-center gap-2 py-3 bg-transparent border border-zinc-700 rounded-md text-slate-300 text-sm font-medium hover:bg-zinc-800 transition-colors">
              <FiEye className="w-4 h-4" /> Approve Comment
            </button>
            <button className="w-full flex items-center justify-center gap-2 py-3 bg-transparent border border-red-500/30 rounded-md text-red-500 text-sm font-medium hover:bg-red-500/10 transition-colors">

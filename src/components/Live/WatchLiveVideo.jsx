...
function WatchLiveVideo({
  ...
  const baseLikes = Number(
    video?.likes ?? video?.liveAnalytics?.liveLikes ?? 0,
  );
  const totalLikes = baseLikes + likeDelta;
  const comments = Number(
    video?.commentsCount ?? video?.liveAnalytics?.liveComments ?? 0,
  );
  ...
  useEffect(() => {
    ...
    if (!initialLikeScanRef.current) {
      likeEntries.forEach((entry) => {
        if (entry?.id) seenLikeIdsRef.current.add(entry.id);
      });
      initialLikeScanRef.current = true;
      return;
    }
    const fresh = likeEntries.filter(
      (entry) => entry?.id && !seenLikeIdsRef.current.has(entry.id),
    );
    if (fresh.length === 0) return;
    fresh.forEach((entry) => {
      seenLikeIdsRef.current.add(entry.id);
    });
  ...
  useEffect(() => {
    ...
    function handleUserUnpublished(user, mediaType) {
      console.info("[LiveViewer] user-unpublished", {
        uid: user?.uid,
        mediaType,
      });
      if (mediaType === "video") {
    ...
    async function connect() {
      try {
        ...
        console.info("[LiveViewer] joining channel", {
        ...
        console.info(
          "[LiveViewer] joined; remote users at join",
          remoteUsers.map((u) => ({
            uid: u.uid,
            hasVideo: u.hasVideo,
            hasAudio: u.hasAudio,
        ...
        if (!cancelled && !remoteUsers.some((u) => u.hasVideo))
          setStreamStatus("waiting");
      } catch (error) {
        if (cancelled) return;
  ...
  return (
    <div className="w-full h-full flex items-center gap-6 md:col-span-3 relative">
      <div
        ...
        className={`absolute z-0 inset-0 md:static md:w-full h-full md:rounded-t-4xl overflow-hidden ${remoteReady ? "flex" : "hidden"}`}
      />
      {/* {!thumb  && <img
        src={thumb}
        alt=""
      ...
      {!remoteReady && isLive && streamStatus !== "idle" ? (
        <div className="absolute z-1 inset-0 md:static md:w-full flex items-center justify-center md:rounded-t-4xl bg-black/40 pointer-events-none">
          <div className="px-4 py-2 rounded-full bg-black/70 text-white text-xs md:text-sm text-center ">
            {streamStatus === "connecting" && "Connecting to the live stream…"}
            {streamStatus === "waiting" && "Waiting for the host's camera…"}
            {streamStatus === "unavailable" &&
              "Live streaming isn't configured on this server yet."}
            {streamStatus === "error" &&
              "Couldn't reach the live stream. Retrying shortly…"}
          </div>
        </div>
      ) : null}
      ...
      <button
        onClick={handleLike}
        type="button"
        className="absolute md:hidden bottom-6 right-10 flex flex-col items-center gap-1"
      >
        <FaRegHeart
          className={`text-black dark:text-white w-5 md:w-8 h-5 md:h-8 ${burstCount > 0 ? "text-red100 dark:text-red100 scale-110 transition-transform" : ""}`}
        />
        <span className="text-[10px] font-semibold text-black dark:text-white">
          {formatCompactNumber(totalLikes)}
        </span>
        {burstCount > 1 && (
          <span className="absolute -top-4 -right-2 text-xs font-bold text-red100">
            +{burstCount}
          </span>
        )}
      </button>
      ...
      <div className="hidden md:flex flex-col gap-3 ">
        <button className="border-2 border-slate150 bg-black dark:bg-white  flex items-center justify-center w-8 h-8 rounded-full">
          <IoMdArrowDropup className="w-5 h-5 text-white dark:text-black" />
        </button>
        <button className="border-2 border-slate150  bg-black dark:bg-white  w-8 h-8 flex items-center justify-center rounded-full">
      ...
      <div className="md:flex flex-col gap-2 z-2 items-center hidden">
        <div className="flex flex-col gap-1 items-center relative">
          ...
          >
            <FaRegHeart
              className={`w-8 h-8 transition-transform ${burstCount > 0 ? "text-red100 scale-110" : "text-black dark:text-white"}`}
            />
          </button>
          ...
          {burstCount > 1 && (
            <span className="absolute -top-3 -right-4 text-sm font-bold text-red100">
              +{burstCount}
            </span>
          )}
        </div>

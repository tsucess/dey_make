...
export function CreateDropdown({isVisible}){
    ...
    return <div className={`flex-col absolute top-12 left-0 rounded-2xl bg-white shadow gap-2.5 w-40 p-4 ${isVisible ? 'flex' : 'hidden'}`}>
        ...
        <Link to="/create-live" className="text-black text-sm flex items-center gap-1.5 hover:text-orange100 transition-all duration-100 hover:scale-105"><MdOutlineSurroundSound className="w-6 h-6"/> {t("Create Live")}</Link>
    </div>
}
  ...
  return (
    <div className="flex flex-col gap-8">
      <div className="flex items-center gap-3">
        {
            tools.map(({title, icon: Icon}, i) => <button onClick={()=> handleActiveToolChange(title)} key={title} className={`transition-all text-sm py-3 px-5 rounded-xl font-semibold flex items-center gap-3 ${
                activeTool === title ? 'bg-orange100 text-black hover:bg-orange200' : 'text-black dark:text-white hover:bg-slate150 hover:dark:bg-black500'
            }`}> <Icon className="w-5 h-5"/> {title}</button>)
        }
      </div>
      {

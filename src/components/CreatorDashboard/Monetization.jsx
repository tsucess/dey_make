...
function Monetization({ summary }) {
  const navigate = useNavigate();
  const payoutAccountReady = Boolean(summary?.payouts?.accountReady);
  const activeMemberships = Number(summary?.memberships?.activeMemberships ?? 0);

  const sources = [
    {
      title: 'Creator Fund',
      desc: 'Earn from video views',
      icon: Cross,
      isOn: payoutAccountReady,
      href: '/coins-wallet',
    },
    {
      title: 'Live Gifts',
      desc: 'Receive gifts during live streams',
      icon: FiGift,
      isOn: payoutAccountReady,
      href: '/coins-wallet',
    },
    {
      title: 'Subscriptions',
      desc: 'Monthly fan subscriptions',
      icon: FaCrown,
      isOn: activeMemberships > 0,
      href: '/coins-wallet',
    },
    {
      title: 'Merch Store',
      desc: 'Sell your branded products',
      icon: IoCartOutline,
      isOn: false,
  ...
  return <div className="flex flex-col gap-5 md:gap-8 font-inter">
    ...
    {
        sources.map(({title, desc, isOn, icon: Icon, href}, i) => <div key={title} className="flex items-center p-3 rounded-2xl bg-white300 dark:bg-black400 bordeer border-black/30 dark:border-white/30 justify-between">
            ...
            <button onClick={()=> navigate(href)} className={`w-16 h-7 p-0.5 rounded-full flex items-center ${isOn ? 'bg-red100 justify-end' : 'bg-slate850/10 dark:bg-slate850/30 justify-start'}`}>

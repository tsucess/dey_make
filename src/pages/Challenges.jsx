...
const CATEGORIES = ["all", "dance", "food", "fitness", "music", "fashion", "photography"];
...
const ACTIVE_CHALLENGES = [
  {
    ...
    description: "Show us your best original dance moves. Best routine wins ₦1,00...",
    participants: "48.2k",
    views: "1.42M",
    prize: "₦500,000",
    daysLeft: "8 days",
  ...
  },
  {
    ...
    category: "PHOTOGRAPHY",
    title: "Cook It Up",
    description: "Make the most creative dish in under 60 seconds. Surprise us...",
    participants: "48.2k",
    views: "1.42M",
    prize: "₦500,000",
    daysLeft: "14 days",
  ...
  },
  {
    ...
    description: "Flaunt your personal style on the streets. Fashion forward, no...",
    participants: "48.2k",
    views: "1.42M",
    prize: "Brand Collab",
  ...
  },
  {
    ...
    category: "PHOTOGRAPHY",
    title: "Fit For 30",
    description: "Document your 30-day fitness transformation journey - daily...",
    participants: "48.2k",
    views: "1.42M",
    prize: "Gym + Pro",
    daysLeft: "22 days",
    joined: true,
    image: "https://images.unsplash.com/photo-1517836357463-d25dfe09ce1e?auto=format&fit=crop&q=80&w=600",
  },
];
    ...
    <div className="min-h-full bg-[#121212] px-4 pb-24 pt-6 md:px-8 md:py-8 text-white">
      <div className="mx-auto w-full max-w-6xl space-y-8">
        ...
        <div className="flex items-center justify-between">
          <h1 className="text-3xl font-bold font-bricolage">{t("challenges.title")}</h1>
        ...
        {/* Hero Section */}
        <div className="relative overflow-hidden rounded-[2rem] bg-gradient-to-r from-purple-900 to-indigo-900 h-48 md:h-56">
          <img 
          ...
          <div className="absolute bottom-0 left-0 flex w-full flex-wrap items-center gap-8 md:gap-16 p-6 md:p-8">
            <div>
              <p className="text-2xl md:text-3xl font-bold text-white">{HERO_STATS.activeNow}</p>
              <p className="text-xs md:text-sm text-slate-300 font-medium">{t("challenges.activeNow")}</p>
            </div>
            <div>
              <p className="text-2xl md:text-3xl font-bold text-yellow-400">{HERO_STATS.prizes}</p>
              <p className="text-xs md:text-sm text-slate-300 font-medium">{t("challenges.inPrizes")}</p>
            </div>
            <div>
              <p className="text-2xl md:text-3xl font-bold text-white">{HERO_STATS.joined}</p>
              <p className="text-xs md:text-sm text-slate-300 font-medium">{t("challenges.joined")}</p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-xl font-bold">{t("challenges.activeChallenges")} <span className="ml-1 inline-flex h-5 w-5 items-center justify-center rounded-full bg-orange100 text-xs text-black">5</span></h2>
            </div>
          ...
          <div className="flex gap-8 overflow-x-auto pb-4 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:'none'] [scrollbar-width:'none'] snap-x">
            {ACTIVE_CHALLENGES.map((challenge) => (
              <div key={challenge.id} className="relative flex w-[280px] shrink-0 snap-start flex-col justify-end overflow-hidden rounded-[1.5rem] bg-[#1A1A1A] h-[380px]">
                <img src={challenge.image} alt={challenge.title} className="absolute inset-0 h-full w-full object-cover opacity-60" />
                ...
                <div className="absolute top-4 left-4 right-4 flex justify-between items-start">
                  {challenge.joined ? (
                    <span className="flex items-center gap-1 rounded bg-red-600 px-2 py-1 text-[10px] font-bold text-white uppercase">
                      <IoCheckmarkCircle className="h-3 w-3" /> {t("challenges.joined")}
                    </span>
                  ) : <div/>}
                ...
                <div className="relative z-10 p-5 pt-0">
                  ...
                  <p className="mb-4 text-xs text-slate-300 line-clamp-2">{challenge.description}</p>
                  
                  <div className="mb-4 flex items-center gap-4 text-xs font-medium text-slate-300">
                    <div className="flex items-center gap-1.5"><FiUsers className="h-3.5 w-3.5"/> {challenge.participants}</div>
                    <div className="flex items-center gap-1.5"><FiEye className="h-3.5 w-3.5"/> {challenge.views}</div>
                  </div>
                  ...
                  <button className={`w-full rounded-full py-2.5 text-sm font-semibold transition-opacity hover:opacity-90 ${challenge.joined ? 'bg-white/20 text-white backdrop-blur-md' : 'bg-orange100 text-black'}`}>
                    {challenge.joined ? t("challenges.viewMyEntry") : t("challenges.joinChallenge")}
                  </button>
                </div>
              </div>
            ))}
        ...
        {/* Trending Bar */}
        <div className="flex items-center justify-between rounded-xl bg-gradient-to-r from-[#3B1515] to-[#251010] border border-red-900/30 px-5 py-3.5">
          <div className="flex items-center gap-3">
            <span className="text-xl">🔥</span>
            <div>
              <p className="text-sm font-bold text-white">#DanceOff2026 {t("challenges.trendingPrefix")}</p>
              <p className="text-xs text-slate-400">1.2M {t("challenges.viewsThisWeek")}</p>
            </div>
          </div>
          <button className="rounded bg-orange100 px-4 py-1.5 text-xs font-bold text-black hover:bg-orange-500">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-bold">{t("challenges.pastChallenges")}</h2>
            <p className="text-xs text-slate-400">{t("challenges.completedCount", { count: 4 })}</p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {PAST_CHALLENGES.map((challenge) => (
              <div key={challenge.id} className="relative flex h-32 w-full overflow-hidden rounded-2xl bg-[#1A1A1A] group cursor-pointer">
                <img src={challenge.image} alt={challenge.title} className="absolute inset-0 h-full w-full object-cover opacity-40 transition-opacity group-hover:opacity-50" />
                ...
                <div className="relative z-10 flex h-full flex-col justify-end p-4">
                  <p className="mb-0.5 text-xs font-bold text-red-500">{challenge.tag}</p>
                  <h3 className="mb-2 text-sm font-bold text-white">{challenge.title}</h3>
                  <div className="flex items-center gap-3 text-[11px] font-medium text-slate-300">
                    <div className="flex items-center gap-1.5"><FiUsers className="h-3 w-3"/> {challenge.entries} {t("challenges.entriesCount")}</div>
                    <div className="flex items-center gap-1.5 text-yellow-400"><IoTrophy className="h-3 w-3"/> {challenge.prize}</div>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="flex flex-col items-center justify-center space-y-3 py-10 text-center">
          ...
          <p className="text-sm text-slate-400">{t("challenges.ideaSubtitle")}</p>

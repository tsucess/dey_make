  ...
  return (
    <section className="flex flex-col gap-10 p-6 font-inter">
      <div className="flex flex-col gap-6 items-center justify-center h-87.5 border border-dashed border-black dark:border-white">
        ...
        <button className="flex items-center justify-center py-3 w-80 bg-orange100 hover:bg-orange500 text-sm rounded-md text-slate100 font-medium transition-all">
          Select video
        </button>
      </div>
      <div className="grid grid-cols-4 gap-4">
        {
            stats.map(({title, desc, icon:Icon}, i) => <div key={title} className="py-8 px-7.5 rounded-2xl flex flex-col gap-8 border border-black/30 dark:border-white/30">
                <div className={`w-12 h-12 flex items-center justify-center border border-black/20 dark:border-white/20 rounded-xl ${
                    i === 0 ? 'bg-black500/10 dark:bg-black500 text-red100':
                    i === 1 ? 'bg-orange100/10 text-orange100' :
                    i === 2 ? 'bg-blue/10 text-blue' : 'text-green100 bg-green100/10'
                }`}> <Icon className="w-6 h-6"/></div>
                <div className="flex flex-col gap-1">
                    <h4 className="font-bold text-base text-black dark:text-white">{title}</h4>
